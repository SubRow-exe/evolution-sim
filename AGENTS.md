...
**Exp09完了・Green。V1.5比較則は事前登録どおり実装されていることが確認済み。
次は「次の1軸を決める」段階であり、新しい世界ルール変更に着手していない。**

V1.6行動則設計案 / Exp10実験計画案が提出され、Claudeレビュー済み
(`docs/V1.6_Exp10_レビュー.md`)。**未採否・未実装。**

レビューで実測から判明した2つの前提のずれは、V1.6着手前に解消が必要:

1. **V1.5は実世界では既にほぼ動いていない** — `sensory_range` 実現値0.29〜0.55で
   感覚半径10〜13.7 wu < `cell_size` 20 wu のため `cr=0`。走査は自セルのみになり、
   Exp09では a/c/e が全5,000 tickで移動量ゼロだった
2. **場が階段状で `ΔQ` がほぼ常に厳密0** — セル内は一定、1セル横断に24.1 tick。
   memory time 1〜30 tickでは約96%のtickで `ΔQ=0` になる

詳細と対応案は `docs/V1.6_Exp10_レビュー.md`。
