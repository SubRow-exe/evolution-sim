...
更新: 2026-08-30
状態: **事前登録 / 実行完了 (2026-08-30) — 結果はケース1**
...
## 6. 実行条件 — 実行完了 (2026-08-30)

結果: `experiments/exp06_actions_20260830_104921/NOTES.md`
Actions run 33307351442 / コード `bdcb825` / 40 run 完走・診断条件チェックGreen。
事前登録どおりの条件で実行し、途中での条件変更は行っていない。

**4条件すべて 0/10 生存 (全滅)。§8 ケース1。**
生存と絶滅の混在は1条件も無いため、seed 11-20 の追加投入は行わない。
...
### ケース1: C (Chem-adapted/Vent) も絶滅  ← **実測はこれ**
