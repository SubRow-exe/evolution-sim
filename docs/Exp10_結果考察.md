...
更新: 2026-09-01
状態: **正式 Phase B 完了 / Green / 人間判断で V1.6 行動原理を採用・Exp10完了**
...
- 行動則は「移動量を一律に増やす」のではなく、同程度の移動の中で空間利用を変える。
  したがって V1.6 は「生存率を直接上げる規則」ではなく、**表現型と環境に応じた
  空間選択を生む規則**として機能している。
- B4 では high-Q / vent 偏在が強まる一方で最終 population が 368→310 に下がった。
  良い局所環境への強い偏在が必ずしも集団規模の増大を意味しないという
  **生態的トレードオフ**が現れており、行動則が単純な適応度ボーナスになっていない。
...
- **§8-3 の +5pp 未達（B1/B3/B5）**：方向は全 seed で正しいが magnitude が小さい。
  ...
  「効かない」とは読まない。
- **high-Q 指標の退化**：B2（および一様場）では `hi_q` が 1.0 固定になり
  §8-3 を測れない。Exp10では結果を見た後に指標を変更せず、B2は事前に観測対象とした
  vent滞在・chemical取得・生存を併記して解釈する。
- **`memory_tau=10` / `response_gain=64` の普遍的最適性**：Phase Aの候補格子と
  事前登録規則の中で選ばれ、Phase Bで機能が確認された実用的な参照値である。
  将来あらゆる環境・進化状態に対して最適であることまでは示していない。
- **長時間での普遍的頑健性**：Phase Cは未実施なので、10,000 tickを大きく超える
  全条件で異常が絶対に起きないことまでは保証しない。

## 7. 人間判断（2026-09-01）

1. **V1.6 の行動原理を採用する（GO）。** Phase Aで局所応答、Phase Bで実生態内の
   空間選択・複数刺激統合・生態非破壊まで確認できたため、次段階へ進む根拠は十分と判断する。
2. `memory_tau=10` / `response_gain=64` は **Exp10で機能確認済みの現行参照値**として
   使用できる。ただし普遍的・進化的な最適値とは扱わず、必要なら将来独立軸として検討する。
3. **Phase C（単なるtick延長の長時間頑健性試験）は現時点では実施しない。**
   Phase A/Bで目的とした挙動が確認でき、追加の長時間実行で新しく答える問いが明確でないため。
   将来、長時間特有の異常や回帰が疑われた場合に再開できるよう「deferred」とする。
4. high-Q指標はExp10後に遡及変更しない。B2の天井効果は既知の測定限界として残す。
5. **Exp10はPhase Bをもって完了扱い**とする。

### 7.1 初回Phase Bで見えた小型化について

進化OFF実装漏れの旧B1/B2は正式なExp10結果ではない。しかし、旧B1で
`body_size` が約0.2へ縮小し個体数が約6,500まで増えた現象は、現行世界で
小型化への強い選択圧が存在する可能性と、将来の計算コスト問題を示す
**exploratoryな設計上の手掛かり**として保持する。

この問題への具体的な世界ルール変更・次バージョン・次実験の内容は、Exp10の結論とは
分離して別途事前検討・事前登録する。