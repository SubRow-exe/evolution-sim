...
### 5.8 補足 — Exp09の選択は「移動」を伴っていない（事後に判明）

V1.6設計案のレビュー（`docs/V1.6_Exp10_レビュー.md` A-1）で判明した重要な補足を、
Exp09の解釈限界としてここへ記録する。**Exp09の判定結果は変わらない**が、
「何を検証したか」の範囲が狭まる。

Exp09の全条件で、個体は実質的に**移動していなかった**。
...
最終snapshotの**全1,201個体が `sensory_range ≤ 0.546`** で、
`cr ≥ 1` に必要な0.8へ誰も届いていない（`sensory_range` は固定遺伝子ではないが、
5,000 tickの間 0.29〜0.55 の範囲に留まった）。
`cr = 0` では最良セルが必ず自セルになるため `stay = True` となり、移動が起きない。

したがってExp09が検証したのは

- **無次元score比較の算術と、その分岐が例外なく使われたこと**

であって、

- **走光性・走化性という空間行動そのもの**

ではない。24,470,222回の選択はすべて自セルを指していた。

§5.5で「V1.5が変えたのは移動先の候補選択」と書いたが、正確には
**この世界設定では候補選択が移動を生んでいない**。
吸収はその場のセルで起きるため、Energy flowの結果（§5.5）はこの事実と矛盾しない。
...
- **走光性・走化性が空間行動として働くか**（§5.8。Exp09では移動が起きていない）
...
### 7.4 感覚半径がセルサイズを下回り、空間探索が発生していない

§5.8のとおり `cr = 0` で走査が自セルのみになり、Exp09では移動が起きていない。
`sensory_range` が 0.8（感覚半径20 wu = 1セル）を超えて初めて近傍セルが見える。

これはV1.5の欠陥ではなく、世界の空間解像度（`cell_size = 20 wu`）と
感覚半径（`sense_coef = 25.0`）と `sensory_range` の実現値の組合せの帰結である。

次の1軸を決めるときの材料になる。**行動則を変える前に、そもそも
現在の世界で空間探索が起きうるパラメータ域なのかを確認する必要がある。**

詳細と対応案は `docs/V1.6_Exp10_レビュー.md` A-1 / A-2 を参照。

### 7.5 chemical-only条件のpopulationが低い
...
Exp09直後は、通常祖先を光+chemical混合世界へ置く長期進化実験を次候補としていた。
しかし §5.8 / §7.4 の追加監査により、現行個体は感覚半径とセルサイズの関係から
ほぼ移動しておらず、Exp09が検証したのは比較算術であって空間行動ではないと判明した。

このため長期混合進化へ直行せず、先にV1.6で一次Energy行動を
temporal biased random walkへ変更し、Exp10で診断・校正する。

正本:
- `docs/V1.6_行動則仕様.md`
- `docs/Exp10_実験計画.md`
- `docs/V1.6_Exp10_レビュー.md`（レビュー原文。採用内容は上記正本を優先）

`chemical_stimulus_half`、他刺激との尺度不整合、chemical bootstrapは未解決点として
保持するが、V1.6/Exp10では同時に変更しない。
