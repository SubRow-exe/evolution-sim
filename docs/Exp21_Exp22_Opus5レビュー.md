...
## 1.2 A0 対照の性質について【要確認】

§3 の「A0 では median 50.0%」は、2通りの読み方がある。

```text
(a) A0 でも出生・死亡が起きているが、long と baseline の差が出なかった
    -> 情報を持つ negative control
(b) A0 では死亡がほとんど起きず、頻度が原理的に動けなかった
    -> 検出力を持たない退化した対照
```

以前の実測では、A0 の固定 iLUCA は初回分裂が約 48 h、120 h でも飢餓死がほとんど発生しない挙動だった。もし A0 の `baseline_deaths_cum` / `long_deaths_cum` がともに 0 に近いなら (b) であり、「A0 で差が出なかった」ことの証拠価値は下がる。

**結果考察へ A0 の出生数・死亡数を明記することを推奨する。** harness は `{lineage}_births_cum` / `{lineage}_deaths_cum` を既に記録しているので、追加実験は不要で、artifact から転記するだけで済む。

(a) であれば §5 の「A0 では利点がほぼ発現しなかった」は強い主張になる。(b) であれば「A0 では頻度が動く機会自体が乏しかった」と書き分けるべきで、環境依存性の主張はやや弱くなる (ただし A2 の 7/8 自体は独立に有効)。
...
```text
- A0 の出生数・死亡数を結果考察へ明記する (対照が検出力を持つかの区別)
